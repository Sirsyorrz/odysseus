...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "workspace_write": "Write a file inside the agent workspace under a project folder — the right tool for building real multi-file code projects (HTML + CSS + JavaScript websites, Python packages with imports, anything where files must reference each other with relative paths). Files land at data/workspace/<project>/ and are served at http://localhost:7000/workspace/<project>/ so the user can preview HTML projects directly.",
    "workspace_read":   "Read a file from a workspace project folder. Use to inspect or modify code the agent previously wrote.",
    "workspace_list":   "List files in a workspace project, or list all projects when project is omitted. Returns paths and sizes.",
    "workspace_delete": "Delete a workspace file, or the entire project folder when path is omitted.",
